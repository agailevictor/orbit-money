...
const OccupationGroupList = (props) => {
    useEffect(() => {
        if (props.occupationGroupList && !props.occupationGroupList.length) fetchOccupationGroups();
    }, []);

    const fetchOccupationGroups = () => {
...
const mapDispatchToProps = (dispatch, getState) => {
    return {
        setData: (data) => dispatch(ActionCreators.occupationGroupAction(data)),
    };
};
export default connect(mapStateToProps, mapDispatchToProps)(OccupationGroupList);
